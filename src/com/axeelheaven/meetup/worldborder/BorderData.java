...
public class BorderData {
	  ...
	  {
	    return "radius " + (this.radiusX == this.radiusZ ? Integer.valueOf(this.radiusX) : new StringBuilder(String.valueOf(this.radiusX)).append("x").append(this.radiusZ).toString()) + " at X: " + BorderConfig.coord.format(this.x) + " Z: " + BorderConfig.coord.format(this.z) + (this.shapeRound != null ? " (shape override: " + BorderConfig.ShapeName(Boolean.valueOf(this.shapeRound.booleanValue())) + ")" : "") + (this.wrapping ? " (wrapping)" : "");
	  }
	  ...
	  {
	    return insideBorder(xLoc, zLoc, false);
	  }
	  
	  public boolean insideBorder(Location loc)
	  {
	    return insideBorder(loc.getX(), loc.getZ(), false);
	  }
	  ...
	  {
	    return insideBorder(coord.x, coord.z, false);
	  }
	  ...
	  public Location correctedPosition(Location loc, boolean round, boolean flying)
	  {
	    ...
	    if (!round)
	    {
	      ...
	      {
	        if (xLoc <= this.minX) {
	          xLoc = this.maxX - BorderConfig.KnockBack();
	        } else if (xLoc >= this.maxX) {
	          xLoc = this.minX + BorderConfig.KnockBack();
	        }
	        if (zLoc <= this.minZ) {
	          zLoc = this.maxZ - BorderConfig.KnockBack();
	        } else if (zLoc >= this.maxZ) {
	          zLoc = this.minZ + BorderConfig.KnockBack();
	        }
	      }
	      else
	      {
	    ...
	    else
	    {
	      ...
	      double f = 1.0D / dT - BorderConfig.KnockBack() / dU;
	      if (this.wrapping)
	      {
	        xLoc = this.x - dX * f;
	        zLoc = this.z - dZ * f;
	  ...
	  {
	    return correctedPosition(loc, false, false);
	  }
	...
	private boolean isSafeSpot(World world, int X, int Y, int Z, boolean flying)
	  ...
	  private double getSafeY(World world, int X, int Y, int Z, boolean flying)
	  {
	    ...
	    int highestBlockBoundary = Math.min(world.getHighestBlockYAt(X, Z) + 1, limTop);
	    ...
	    int y1 = Y;
	    for (int y2 = Y; (y1 > 0) || (y2 < limTop); y2++)
	    {
	      if (y1 > 0) {
